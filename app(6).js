...
function normalizeThaiDigits(text) {
    ...
    return String(text).replace(/[๐-๙]/g, digit => String(thaiDigits.indexOf(digit)));
}

function cleanOCRText(text) {
    return normalizeThaiDigits(String(text || '')).replace(/\r/g, '').replace(/[ \t]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim();
}
// ============================================================
// 5. UI INITIALIZATION
// ============================================================
...
async function saveManualTransaction() {
    ...
    const type = document.getElementById('fType')?.value || 'cash';
    ...
    save();
    addLog('add_transaction', transaction);
...
function normalizeThaiDigits(text) {
    ...
    return String(text).replace(/[๐-๙]/g, d => thaiDigits[d]);
}
...
function initUI() {
    ...
    if (catGrid) {
        catGrid.innerHTML = CATEGORIES.map(category => `
            ...
            `).join('');
        catGrid.querySelectorAll('.cat-item').forEach(element => {
    ...
    if (resCat) {
        resCat.innerHTML = CATEGORIES.map(category => `
...
function initNavigation() {
    ...
    document.querySelectorAll('[data-page]').forEach(btn => {
        btn.addEventListener('click',
            () => {
                ...
                if (dropdownMenu && dropdownMenu.contains(btn)) {
                    dropdownBtn?.classList.add('active');
                } else {
                    dropdownMenu?.classList.remove('show');
                }
                const underlineBtn = dropdownMenu && dropdownMenu.contains(btn) ? dropdownBtn : btn;
                updateUnderline(underlineBtn);
                renderAll();
            });
    });
    ...
    setTimeout(
        () => {
            updateUnderline(tabs?.querySelector('button.active'));
        }, 100);
}
// ============================================================
// 8. INCOME
...
function initIncomeInput() {
    ...
    incomeInput?.addEventListener('change', async event => {
        ...
        if (value !== oldValue) {
            const confirmed = await confirmAction('ยืนยันรายรับ?', `รายรับเดือนนี้ ฿${formatMoney(value)}`, 'บันทึกรายรับ');
            if (!confirmed) {
        ...
        renderAll();
        showNotification('บันทึกรายรับเรียบร้อย', 'success');
    });
}
// ============================================================
...
function initManualForm() {
    ...
    form.addEventListener('submit', async event => {
        ...
        const confirmed = await confirmAction('ยืนยันการบันทึกรายการ?', `วันที่ ${date}
จำนวนเงิน ฿${formatMoney(amount)}
หมวดหมู่ ${category?.label || 'อื่นๆ'}
${note || 'ไม่มีรายละเอียด'}`, 'บันทึกรายการ');
        if (!confirmed) {
            return;
        }
        const tx = {
        ...
        if (uploadTxt) {
            uploadTxt.textContent = '📷 แตะเพื่อแนบรูปสลิปเก็บไว้';
        }
        renderAll();
        await notifySuccess('บันทึกรายการเรียบร้อย', `฿${formatMoney(amount)}`);
    });
...
function initReceiptUpload() {
    ...
    uploadBox?.addEventListener('click',
        () => fReceipt?.click());
    fReceipt?.addEventListener('change', async event => {
        const file = event.target.files?. [0];
        if (!file) return;
        if (!file.type.startsWith('image/')) {
            await notifyError('ไฟล์ไม่ถูกต้อง', 'กรุณาเลือกไฟล์รูปภาพ');
            return;
        }
        manualReceiptBase64 = await compressImage(file, 800, 0.8);
        const uploadTxt = document.getElementById('uploadTxt');
        if (uploadTxt) {
            uploadTxt.textContent = `✅ แนบไฟล์เรียบร้อย (${file.name})`;
        }
    });
}
// ============================================================
...
function initTaxButton() {
    document.getElementById('taxCalcBtn')?.addEventListener('click', async () => {
        ...
        if (!income) {
            await notifyWarning('ยังไม่ได้กรอกรายได้', 'กรุณากรอกรายได้ทั้งปีก่อน');
            return;
        }
        const confirmed = await confirmAction('ยืนยันการคำนวณภาษี?', `รายได้ทั้งปี ฿${formatMoney(income)}`, 'คำนวณภาษี');
        if (confirmed) {
            calculateTax();
        }
    });
...
function initSlipScanner() {
    ...
    dropzone.addEventListener('dragover', event => {
        event.preventDefault();
        dropzone.style.borderColor = 'var(--good)';
    });
    ...
    dropzone.addEventListener('drop', async event => {
        ...
        if (!files.length) {
            await notifyWarning('ไม่พบรูปภาพ', 'กรุณาลากไฟล์รูปภาพสลิป');
            return;
        }
        if (files.length === 1) {
...
// 13. SINGLE OCR
// ============================================================
async function enhanceSlipForOCR(base64) {
    return new Promise(
        (resolve, reject) => {
...
function mergeOCRTexts(primary, enhanced) {
    const first = String(primary || '').trim();
    const second = String(enhanced || '').trim();
    if (!second) {
        return first;
    }
    if (!first) {
...
function mergeKBankParsedResults(results) {
    ...
    const base = valid[0] || {};
    const amountValues = valid.map(item => Number(item.amount || 0)).filter(value => Number.isFinite(value) && value > 0);
    ...
    amountValues.forEach(value => {
        ...
        amountCounts.set(key,
            (amountCounts.get(key) || 0) + 1);
    });
    const consensusAmount = [...amountCounts.entries()].sort(
        (a, b) => b[1] - a[1])[0]?. [0];
    const namesSender = valid.map(item => item.sender).filter(Boolean);
    const namesReceiver = valid.map(item => item.receiver).filter(Boolean);
    const sender = namesSender.length ? namesSender.reduce(
        (best, name) => chooseBetterPersonName(best, name, 'sender'), '') : '';
    const receiver = namesReceiver.length ? namesReceiver.reduce(
        (best, name) => chooseBetterPersonName(best, name, 'receiver'), '') : '';
    const dates = valid.map(item => item.date).filter(Boolean);
    const times = valid.map(item => item.time).filter(Boolean);
    const date = dates.find(d => dates.filter(x => x === d).length >= 2) || dates[0] || '';
    const time = times.find(t => times.filter(x => x === t).length >= 2) || times[0] || '';
    const best = valid.slice().sort(
        (a, b) => {
            const score = item => (item.sender ? 4 : 0) + (item.receiver ? 4 : 0) + (item.amount ? 4 : 0) + (item.date ? 2 : 0) + (item.time ? 1 : 0);
...
async function processSlipImage(file) {
    ...
    if (preview) {
        preview.src = currentScannedImageBase64;
    }
    if (dropzone) {
    ...
    try {
        if (progress) {
            progress.textContent = 'กำลังอ่านข้อความบนสลิป...';
        }
        ...
        const primaryParsed = parseSlipOCRText(primaryText);
        let parsed = primaryParsed;
        let allTexts = [primaryText];
        // KBank: อ่านซ้ำทุกครั้งที่ข้อมูลสำคัญยังไม่ครบ
        // และอ่านเฉพาะบริเวณชื่อเพื่อป้องกัน OCR ของเลขอ้างอิง/บัญชีมาปนกับชื่อ
        if (primaryParsed.bank === 'kbank') {
            const parsedResults = [primaryParsed];
            if (progress) {
                progress.textContent = 'กำลังตรวจสอบยอดและชื่อผู้โอน/ผู้รับ KBank...';
            }
            try {
                ...
                allTexts.push(enhancedText);
                parsedResults.push(parseSlipOCRText(enhancedText));
            } catch (error) {
                console.warn('KBank enhanced OCR failed:', error);
            }
            try {
                const personImage = await createKBankPersonOCRImage(currentScannedImageBase64);
                const personResult = await worker.recognize(personImage);
                const personText = personResult?.data?.text || '';
                allTexts.push(personText);
                parsedResults.push(parseSlipOCRText(personText));
            } catch (error) {
                console.warn('KBank person-region OCR failed:', error);
            }
            // ใช้ consensus/คะแนน ไม่ใช่รอบล่าสุดทับรอบแรก
            parsed = mergeKBankParsedResults(parsedResults);
            parsed.rawText = mergeOCRTexts(primaryText, allTexts.slice(1).join('\n'));
        }
        if (worker) {
        ...
        if (progress) {
            progress.textContent = 'กำลังตรวจสอบข้อมูลก่อนแสดงผล...';
        }
...
function fillSingleScanResult(data) {
    ...
    const sender = document.getElementById('resSender');
    const receiver = document.getElementById('resReceiver');
    const note = document.getElementById('resNote');
    if (amount) {
        amount.value = data.amount || '';
    }
    if (date) {
        date.value = data.date || todayStr();
    }
    if (time) {
    ...
    if (note) {
        note.value = data.note || 'โอนผ่านสลิปธนาคาร';
    }
}
// ============================================================
// 15. SAVE SINGLE OCR
...
async function saveSingleScannedReceipt() {
    ...
    const confirmed = await confirmAction('ยืนยันบันทึกสลิป?', `จำนวนเงิน ฿${formatMoney(amount)}
วันที่ ${date}
${time ? `เวลา ${time}` : ''}
${sender ? `ผู้โอน: ${sender}` : ''}
${receiver ? `ผู้รับ: ${receiver}` : ''}`, 'บันทึกสลิป');
    if (!confirmed) {
        return;
    }
    const tx = {
...
function resetScanUI() {
    ...
    const fields = ['resAmount', 'resDate', 'resTime', 'resSender', 'resReceiver', 'resNote'];
    fields.forEach(id => {
...
// ============================================================
async function createKBankPersonOCRImage(base64) {
    return new Promise(
        (resolve, reject) => {
            const img = new Image();
            img.onload = () => {
...
function normalizePersonName(name) {
     ...
     */
    value = value.replace(/^(ผู้โอน|ผู้ส่ง|ผู้ชำระ|ผู้จ่าย|จาก|sender)\s*[:：\-]?\s*/i, '');
    value = value.replace(/^(ผู้รับ|ผู้รับเงิน|ผู้รับโอน|ถึง|receiver)\s*[:：\-]?\s*/i, '');
    /*
     ...
     */
    const invalidWords = ['kbank', 'kasikorn', 'กสิกรไทย', 'ธนาคารกสิกรไทย', 'promptpay', 'พร้อมเพย์', 'reference', 'transaction', 'หมายเลขรายการ', 'เลขที่รายการ', 'บัญชี', 'account', 'ref'];
...
// ============================================================
function chooseBetterPersonName(current, candidate, role = '') {
    ...
    if (al.includes(bl) && a.length >= b.length) {
        return a;
    }
    if (bl.includes(al) && b.length >= a.length) {
        return b;
    }
    /*
    ...
    function scoreName(name) {
         ...
         * มีคำนำหน้าชื่อ
         */
        if (/^(นาย|นาง|นางสาว|เด็กชาย|เด็กหญิง|mr\.?|mrs\.?|ms\.?)/i.test(name)) {
            score += 3;
        }
        /*
         * มีตัวอักษรไทย
...
function extractPerson(text, labels) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const label of labels) {
            ...
            if (line.toLowerCase().includes(label.toLowerCase())) {
                ...
                const name = normalizePersonName(next);
                if (name && !/\d{4,}/.test(next)) {
                    candidates.push(name);
                }
            }
        }
    ...
    return candidates.reduce(
        (best, candidate) => chooseBetterPersonName(best, candidate), '');
}
// ============================================================
// 22. KBank PERSON EXTRACTION
// ============================================================
function extractKBankPersons(text) {
    const normalized = normalizeOCRText(text);
    const lines = normalized.split(/\r?\n/).map(line => line.trim()).filter(Boolean);
    let sender = '';
    let receiver = '';
     ...
     * label ตรง ๆ
     */
    sender = extractPerson(normalized,
        ['ผู้โอน', 'ผู้ส่ง', 'ผู้ชำระ', 'ผู้จ่าย', 'จาก', 'sender', 'from']);
    receiver = extractPerson(normalized,
        ['ผู้รับ', 'ผู้รับเงิน', 'ผู้รับโอน', 'ถึง', 'receiver', 'to']);
    /*
     * วิธีที่ 2:
    ...
    return {
        sender: normalizePersonName(sender),
        receiver: normalizePersonName(receiver)
    };
}
// ============================================================
// 23. BANK DETECTION
...
function detectBank(text) {
    ...
    if (lower.includes('kbank') || lower.includes('kasikorn') || lower.includes('กสิกร') || lower.includes('กสิกรไทย') || lower.includes('k plus') || lower.includes('k+') || lower.includes('make')) {
        return 'kbank';
    }
    return 'unknown';
}
...
function extractBestAmount(text, bank = 'unknown') {
    ...
    function addCandidate(raw, score, sourceName) {
        if (!raw) {
            return;
        }
        let cleaned = String(raw).trim().replace(/฿/g, '').replace(/บาท/gi, '').replace(/[\s,]/g, '');
        ...
        if (!Number.isFinite(value) || value <= 0 || value >= 5000000) {
            return;
        }
        /*
         ...
         */
        if (Number.isInteger(value) && !/[฿]|บาท|,\d{3}|\.\d{2}/.test(String(raw))) {
            score -= 18;
        }
        candidates.push({
            value,
    ...
    if (bank === 'kbank') {
        ...
        lines.forEach(
            (line, index) => {
                ...
                const hasAmountLabel = amountLabels.some(label => lower.includes(label));
                if (!hasAmountLabel) {
                    return;
                }
                /*
                 ...
                 */
                const sameLineMatches = line.match(/(?:฿\s*)?[\d][\d,\s]*(?:\.\d{1,2})?(?=\s*(?:บาท|baht|thb)?\b)/gi) || [];
                sameLineMatches.forEach(raw => addCandidate(raw, 250, 'kbank-label-same-line'));
                /*
                ...
                for (let offset = 1; offset <= 2; offset++) {
                     ...
                     */
                    if (/เลขที่|เลขอ้างอิง|reference|ref|transaction|บัญชี|account|ค่าธรรมเนียม|fee|พร้อมเพย์|promptpay|วันที่|เวลา|date|time/i.test(next)) {
                        continue;
                    }
                    const moneyMatches = next.match(/(?:฿\s*)?[\d][\d,\s]*(?:\.\d{1,2})(?:\s*(?:บาท|baht|thb))?/gi) || [];
                    moneyMatches.forEach(raw => addCandidate(raw, offset === 1 ? 235 : 215, 'kbank-label-next-line'));
                    /*
                     * MAKE บางรูปแบบอาจไม่มี .00
                     * แต่มี comma เช่น 8,000
                     */
                    if (!moneyMatches.length && /[฿,]|\bบาท\b|\bbaht\b|\bthb\b/i.test(next)) {
                        const integerMatches = next.match(/(?:฿\s*)?\d[\d,\s]*/g) || [];
                        integerMatches.forEach(raw => addCandidate(raw, 190, 'kbank-label-integer'));
                    }
                }
            });
    }
    ...
    for (const match of source.matchAll(moneyRegex)) {
        ...
        let score = 30;
        if (/จำนวนเงิน|ยอดเงิน|ยอดโอน|amount|total|payment|ชำระ|บาท|baht|thb/.test(before + after)) {
            score += 80;
        }
        if (/ค่าธรรมเนียม|fee|service|commission/.test(before + after)) {
            score -= 100;
        }
        if (/เลขที่|เลขอ้างอิง|reference|ref|transaction|บัญชี|account|พร้อมเพย์|promptpay/.test(before + after)) {
            score -= 100;
        }
        /*
         * วัน/เดือน/ปี และเวลาไม่ใช่จำนวนเงิน
         */
        if (/วันที่|date|\b20\d{2}\b|\b25\d{2}\b|เวลา|time/.test(before + after)) {
            score -= 90;
        }
        score += 25;
        addCandidate(match[1], score, 'generic-decimal');
    }
    /*
     * Integer fallback:
     * ใช้เฉพาะเมื่อมีหลักฐานว่าเป็นเงินจริง
     */
    const integerRegex = /(?:฿\s*)?(\d{1,3}(?:[,\s]\d{3})+|\d{4,})/g;
    for (const match of source.matchAll(integerRegex)) {
        const before = source.slice(Math.max(0, match.index - 70), match.index).toLowerCase();
        const after = source.slice(match.index, match.index + 70).toLowerCase();
        let score = 5;
        if (/จำนวนเงิน|ยอดเงิน|ยอดโอน|amount|total|บาท|baht|thb|ชำระ|โอน/.test(before + after)) {
            score += 55;
        }
        if (/ค่าธรรมเนียม|fee|service|เลขที่|reference|ref|transaction|บัญชี|account|promptpay|พร้อมเพย์/.test(before + after)) {
            score -= 100;
        }
        if (/วันที่|date|เวลา|time|\b20\d{2}\b|\b25\d{2}\b/.test(before + after)) {
            score -= 100;
        }
        /*
         ...
         */
        if (bank === 'kbank' && !/[฿,]|บาท|baht|thb/i.test(match[0] + before.slice(-30) + after.slice(0, 30))) {
            score -= 35;
        }
        addCandidate(match[1], score, 'generic-integer');
    }
    if (!candidates.length) {
        return null;
    }
    ...
    candidates.forEach(candidate => {
        const existing = unique.find(item => Math.abs(item.value - candidate.value) < 0.001);
        if (existing) {
     ...
     */
    if (bank === 'kbank') {
        const kbankCandidate = unique.filter(candidate => candidate.sources.some(source => source.startsWith('kbank-label'))).sort(
...
function extractDate(text) {
     ...
     * 2) Numeric date
     */
    const patterns = [/(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{4})/, /(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{2})/];
...
function cleanOCRPersonName(value) {
     ...
     */
    result = result.replace(/\b(?:promptpay|พร้อมเพย์|เลขที่รายการ|เลขที่อ้างอิง|reference|transaction|account|บัญชี)\b.*$/i, '').trim();
    /*
     * ไม่รับบรรทัดที่เป็นเลขล้วน/เบอร์โทร/บัญชี
     */
    const compact = result.replace(/[\s\-xX]/g, '');
    if (!result || compact.length < 2 || /^\d+$/.test(compact) || /^\d{5,}$/.test(compact)) {
        return '';
    }
    /*
     * ชื่อบุคคลไม่ควรมีจำนวนเงิน/วันที่
     */
    if (/\b\d[\d,]*\.\d{2}\b/.test(result) || /\b(?:20|25)\d{2}\b/.test(result)) {
        return '';
    }
    return result;
}

function isLikelyKBankAccountLine(line) {
    const value = String(line || '').trim();
    if (!value) {
        return false;
    }
    const compact = value.replace(/[\s\-xX]/g, '');
    return (/^\d{6,}$/.test(compact) || /^x{2,}\d{2,}$/i.test(compact) || /^\d{2,4}[-\s]x{2,}\d{2,}$/i.test(value) ||
        /*
         * K PLUS / MAKE มัก mask บัญชีเป็น
         * xxx-x-x0874-x หรือ xxx-x-x8348-x
         */
        /^(?:x{2,}|\d{1,4})[-\s]?(?:x{1,}|\d{1,4})[-\s]?(?:x{1,}|\d{1,6})[-\s]?(?:x{1,}|\d{1,4})$/i.test(value));
}

function isLikelyPersonLine(line) {
    const value = cleanOCRPersonName(line);
    if (!value) {
        return false;
    }
    if (isLikelyKBankAccountLine(value)) {
        return false;
    }
    if (/^(โอนเงินสำเร็จ|สำเร็จ|จำนวนเงิน|จำนวน|ค่าธรรมเนียม|เลขที่รายการ|เลขที่อ้างอิง|promptpay|พร้อมเพย์|scan|ตรวจสอบ|วันที่|เวลา|amount|total)$/i.test(value)) {
        return false;
    }
    /*
     * ห้ามเอาชื่อธนาคาร / ชื่อระบบมาเป็นชื่อบุคคล
     */
    if (/ธนาคาร|กสิกรไทย|kasikorn|kbank|make\s*by|promptpay|พร้อมเพย์/i.test(value)) {
        return false;
    }
    if (/^(?:฿\s*)?[\d,\s.]+(?:บาท|baht|thb)?$/i.test(value)) {
        return false;
    }
    /*
     * ชื่อควรมีตัวอักษรไทยหรืออังกฤษ
     */
    if (!/[ก-๙A-Za-z]/.test(value)) {
        return false;
    }
    return (value.length >= 2 && value.length <= 80);
}

function extractKBankPeople(text) {
     ...
     */
    if (arrowIndexes.length) {
        const arrowIndex = arrowIndexes[0];
        /*
         * receiver มักอยู่หลังลูกศร 1-3 บรรทัด
         */
        for (let offset = 1; offset <= 3; offset++) {
            const candidate = lines[arrowIndex + offset];
            if (isLikelyPersonLine(candidate)) {
                receiver = candidate;
                break;
            }
        }
         ...
         */
        for (let i = arrowIndex - 1; i >= Math.max(0, arrowIndex - 5); i--) {
    ...
    if (!sender || !receiver) {
        ...
        for (let i = 0; i < lines.length; i++) {
             ...
             */
            const hasAccountAfter = isLikelyKBankAccountLine(next);
            const hasBankAfter = /กสิกร|kasikorn|kbank|make|promptpay|พร้อมเพย์/i.test(next);
            const hasArrowBefore = /[↓→➜➝]/.test(previous);
            let score = 0;
            if (hasAccountAfter) {
                score += 80;
            }
        ...
        if (!sender) {
            sender = personCandidates.find(candidate => candidate.index < (arrowIndexes[0] ?? lines.length))?.name || '';
        }
        if (!receiver) {
            receiver = personCandidates.find(candidate => candidate.name !== sender && (candidate.index > (arrowIndexes[0] ?? -1)))?.name || '';
        }
    }
    /*
     ...
     */
    if (!sender) {
        sender = extractPerson(text,
            ['ผู้โอน', 'จาก', 'sender', 'from']);
    }
    if (!receiver) {
        receiver = extractPerson(text,
            ['ผู้รับ', 'ถึง', 'receiver', 'to']);
    }
    return {
        sender: cleanOCRPersonName(sender),
        receiver: cleanOCRPersonName(receiver)
    };
}
// ============================================================
...
function extractPerson(text, labels) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const label of labels) {
            ...
            const match = line.match(regex);
            if (match && match[1]) {
                const value = cleanOCRPersonName(match[1]);
                if (isLikelyPersonLine(value)) {
                    return value;
                }
            }
            if (line.toLowerCase().includes(label.toLowerCase())) {
                const next = lines[i + 1] || '';
                const value = cleanOCRPersonName(next);
                if (isLikelyPersonLine(value)) {
                    return value;
                }
            }
...
function detectCategory(text) {
     ...
     */
    const lines = normalizeOCRText(text).split(/\r?\n/).map(line => line.trim()).filter(Boolean);
    const meaningfulLines = lines.filter(line => !/เลขที่รายการ|เลขที่อ้างอิง|reference|transaction|account|บัญชี|promptpay|พร้อมเพย์|xxx[-\s]|^\d{5,}$/i.test(line)).join(' ');
    const lower = meaningfulLines.toLowerCase();
    if (
        ['เซเว่น', '7-eleven', 'อาหาร', 'food', 'grab', 'lineman', 'ก๋วยเตี๋ยว', 'ร้านอาหาร', 'cafe', 'coffee', 'restaurant', 'mcdonald'].some(keyword => lower.includes(keyword)) || /\bkfc\b/i.test(lower)) {
        return {
            category: 'food',
    ...
    if (
        ['ptt', 'mrt', 'bts', 'น้ำมัน', 'ทางด่วน', 'ปตท', 'taxi', 'grabcar', 'bolt'].some(keyword => lower.includes(keyword))) {
        return {
            category: 'transport',
...
function detectCategory(text) {
    ...
    if (
        ['pea', 'mea', 'ไฟฟ้า', 'ประปา', 'ais', 'true', 'dtac', 'ค่าไฟ', 'ค่าน้ำ', 'internet', 'โทรศัพท์'].some(keyword => lower.includes(keyword))) {
        return {
            category: 'bills',
    ...
    if (
        ['shopee', 'lazada', 'tiktok', 'mall', 'central', 'shopping'].some(keyword => lower.includes(keyword))) {
        return {
            category: 'shopping',
            note: 'ชอปปิงออนไลน์'
        };
...
function shiftMonth(delta) {
    ...
    state.currentMonth = `${date.getFullYear()}-${String(
            date.getMonth() + 1
        ).padStart(2, '0')}`;
    save();
    renderAll();
...
function renderMonthLabels() {
    ...
    label.textContent = `${thaiMonths[date.getMonth()]} ${date.getFullYear() + 543}`;
}
// ============================================================
// 31. DASHBOARD
// ============================================================
function renderDashboard() {
    ...
    const monthTransactions = state.transactions.filter(tx => String(tx.date || '').startsWith(state.currentMonth));
    ...
    if (countEl) {
        countEl.textContent = String(monthTransactions.length);
    }
}
// ============================================================
// 32. TRANSACTIONS
...
function renderTransactions() {
    ...
    const monthTransactions = state.transactions.filter(tx => String(tx.date || '').startsWith(state.currentMonth)).sort(
        (a, b) => (String(b.date || '') + String(b.time || '')).localeCompare(String(a.date || '') + String(a.time || '')));
...
function renderIncomeExpenseChart() {
    ...
    const income = Number(state.income[state.currentMonth] || 0);
    const expense = state.transactions.filter(tx => String(tx.date || '').startsWith(state.currentMonth)).reduce(
        (sum, tx) => sum + Number(tx.amount || 0), 0);
    /*
    ...
    const barWidth = Math.min(80, width / 4);
    const values = [{
        label: 'รายรับ',
        value: income
    }, {
    ...
    values.forEach(
        (item, index) => {
            const x = width / 2 - barWidth - 10 + index * (barWidth + 20);
            const barHeight = (item.value / maxValue) * chartHeight;
            const y = height - padding - barHeight;
            ctx.fillStyle = index === 0 ? '#16a34a' : '#ef4444';
            ctx.fillRect(x, y, barWidth, barHeight);
            ctx.fillStyle = '#333';
            ctx.textAlign = 'center';
            ctx.font = '14px sans-serif';
            ctx.fillText(item.label, x + barWidth / 2, height - 12);
            ctx.fillText(`฿${formatMoney(
                    item.value
                )}`, x + barWidth / 2, Math.max(18, y - 8));
        });
}
year,
month - 1 + delta,
...
function renderMonthLabels() {
    ...
    const date = new Date(year, month - 1, 1);
    const text = date.toLocaleDateString('th-TH', {
        month: 'long',
        year: 'numeric'
    });
...
function renderDashboard() {
    ...
    const expense = monthTx.reduce(
        (sum, tx) => sum + Number(tx.amount || 0), 0);
    const remaining = income - expense;
    const expenseElement = document.getElementById('totalExpenseVal');
    const remainingElement = document.getElementById('remainingVal');
    if (expenseElement) {
        expenseElement.textContent = `฿${formatMoney(expense)}`;
    }
    if (remainingElement) {
        remainingElement.textContent = `฿${formatMoney(remaining)}`;
        remainingElement.style.color = remaining >= 0 ? 'var(--good)' : 'var(--danger)';
    }
...
function renderTransactions() {
    ...
    const transactions = state.transactions.filter(tx => String(tx.date || '').startsWith(state.currentMonth)).sort(
    ...
    Object.entries(grouped).forEach(
        ([date, items]) => {
            ...
            html += `
                ...
                `;
            items.forEach(tx => {
                const category = CATEGORIES.find(c => c.id === tx.category) || CATEGORIES[CATEGORIES.length - 1];
                html += `
...
async function deleteTx(id) {
    ...
    const confirmed = await confirmAction('ยืนยันการลบรายการ?', `จำนวนเงิน ฿${formatMoney(transaction.amount)}
${transaction.note || ''}

การลบรายการไม่สามารถย้อนกลับได้`, 'ลบรายการ');
    if (!confirmed) {
        return;
    }
    ...
    renderAll();
    await notifySuccess('ลบรายการเรียบร้อย');
}
// ============================================================
// 35. ANALYSIS
// ============================================================
function renderAnalysis() {
    ...
    const expense = transactions.reduce(
        (sum, tx) => sum + Number(tx.amount || 0), 0);
    const needs = transactions.filter(tx => ['food', 'transport', 'bills'].includes(tx.category)).reduce(
        (sum, tx) => sum + Number(tx.amount || 0), 0);
    const wants = transactions.filter(tx => ['shopping', 'entertainment', 'other'].includes(tx.category)).reduce(
        (sum, tx) => sum + Number(tx.amount || 0), 0);
    ...
    const list = [];
    if (!income) {
        list.push('กรุณากรอกรายรับเดือนนี้ในหน้าภาพรวม');
    } else {
        if (needs > income * 0.5) {
            list.push('ค่าใช้จ่ายจำเป็นเกิน 50% ของรายได้');
        }
        if (wants > income * 0.3) {
            list.push('ค่าใช้จ่ายด้านความต้องการเกิน 30%');
        }
        if (savings >= income * 0.2) {
            list.push('ยอดเยี่ยม! ออมได้อย่างน้อย 20%');
        } else {
            list.push('พยายามเพิ่มเงินออมให้ถึง 20%');
        }
    }
...
// 36. BAR
// ============================================================
function renderBar(label, actual, target, savings = false) {
    const percent = target > 0 ? Math.min(100, actual / target * 100) : 0;
    const over = !savings && actual > target;
    return `
...
function renderLogs() {
    ...
    list.innerHTML = state.logs.slice(-15).reverse().map(log => `
...
function exportLogsCSV() {
    if (!state.logs || !state.logs.length) {
        notifyWarning('ยังไม่มีประวัติให้ส่งออก');
        return;
    }
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `finance-logs-${new Date()
            .toISOString()
            .slice(0, 10)}.csv`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    notifySuccess('ส่งออก Logs เรียบร้อย');
}
// ============================================================
// 43. TAX CALCULATOR
// ============================================================
function openTaxCalculator() {
    ...
    const totalExpense = state.transactions.reduce(
        (sum, tx) => sum + Number(tx.amount || 0), 0);
    const taxableBase = Math.max(0, totalIncome - totalExpense);
    /*
     * คำนวณแบบประมาณการอย่างง่าย
     */
    const deduction = 100000;
    const netIncome = Math.max(0, taxableBase - deduction);
    let tax = 0;
    if (netIncome <= 150000) {
        tax = 0;
...
async function runSystemTest() {
     ...
     */
    try {
        const testKey = '__finance_test__';
        localStorage.setItem(testKey, 'ok');
        const value = localStorage.getItem(testKey);
        localStorage.removeItem(testKey);
        results.push({
            name: 'LocalStorage',
            status: value === 'ok' ? 'ผ่าน' : 'ไม่ผ่าน'
        });
    } catch (error) {
        results.push({
            name: 'LocalStorage',
    ...
    results.push({
        name: 'Tesseract.js',
        status: typeof Tesseract !== 'undefined' ? 'พร้อมใช้งาน' : 'ไม่พบ'
    });
    /*
     * SweetAlert
     */
    results.push({
        name: 'SweetAlert2',
        status: typeof Swal !== 'undefined' ? 'พร้อมใช้งาน' : 'ไม่พบ'
    });
    /*
     * DOM สำคัญ
     */
    const requiredIds = ['fDate', 'fAmount', 'fNote', 'catGrid', 'txList', 'incomeInput'];
    const missing = requiredIds.filter(id => !document.getElementById(id));
    results.push({
        name: 'DOM',
        status: missing.length ? `ขาด ${missing.join(', ')}` : 'ครบ'
    });
...
// labels
if (count <= 31 || i % Math.ceil(count / 12) === 0) {
    ctx.fillStyle = '#374151';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    ctx.font = '10px Sarabun, sans-serif';
    ctx.fillText(data.labels[i], center, padding.top + chartHeight + 8);
}
}
// Legend
...
ctx.fillStyle = '#374151';
ctx.fillText('รายรับ', padding.left + 15, 14);
ctx.fillStyle = '#E53935';
ctx.fillRect(padding.left + 70, 5, 10, 10);
ctx.fillStyle = '#374151';
ctx.fillText('รายจ่าย', padding.left + 85, 14);
}
// ============================================================
// 40. CHART SUMMARY
// ============================================================
function renderChartSummary(data) {
    const element = document.getElementById('chartSummary');
    if (!element) {
        return;
    }
    const income = data.income.reduce(
        (sum, value) => sum + value, 0);
    const expense = data.expense.reduce(
        (sum, value) => sum + value, 0);
    const remaining = income - expense;
    element.innerHTML = `
...
// 40. IMAGE COMPRESS
// ============================================================
function compressImage(file, maxSide = 1400, quality = 0.92) {
    return new Promise(
        (resolve, reject) => {
            const reader = new FileReader();
            reader.onerror = reject;
...
function exportLogsCSV() {
    ...
    state.logs.forEach(log => {
        csv += `"${csvEscape(log.timestamp)}",` + `"${csvEscape(log.action)}",` + `"${csvEscape(
                    JSON.stringify(
                        log.details || {}
                    )
                )}"\n`;
...
function runSystemDiagnostics() {
    ...
    `;
    notifySuccess('ทดสอบระบบเสร็จแล้ว');
}
// ============================================================
// 44. ESCAPE HTML
// ============================================================
...
function initEnhancements() {
    ...
    window.addEventListener('resize',
        () => {
            ...
            window.__financeResizeTimer = setTimeout(
                () => renderIncomeExpenseChart(), 150);
        });
    const saveAll = document.getElementById('saveAllScansBtn');
    saveAll?.addEventListener('click', saveAllScannedReceipts);
    const clearAll = document.getElementById('clearAllScansBtn');
    clearAll?.addEventListener('click', clearMultiScanResultsConfirm);
    const multiSelect = document.getElementById('multiScanSelectBtn');
